...
<script lang="ts">
	...
	let requiresAction = $derived(!widgetState.isConnected || !subscriptionStatus.isActive);
</script>
...
<div class="subscription-tab">
	{#if status.isActive}
		<div class="subscription-status">
			...
			<button onclick={() => subscriptionManager.cancel()}> Cancel Subscription </button>
		</div>
	{:else}
		<div class="subscription-plans">
			...
			<button onclick={subscribe} disabled={!selectedPlan || isProcessing}>
	...
	return (
		<div>
			{hasSubscription ? (
				...
				<button onClick={() => window.WalletKit.promptSubscribe()}>Unlock Premium Features</button>
			)}
		</div>
	);
}
